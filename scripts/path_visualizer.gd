...
func _ready() -> void:
	...
	mat_marker.emission_energy_multiplier = 1.8
...
func draw_path(waypoints: Array[Vector3], color: Color = Color(0.1, 0.9, 0.55)) -> void:
	...
	for i in range(waypoints.size()):
		...
		marker.position = Vector3(wp.x, 0.08, wp.z)
		add_child(marker)

		# Connecting line segment to next waypoint
		if i < waypoints.size() - 1:
			...
			var box = BoxMesh.new()
			box.size = Vector3(0.08, 0.04, dist)
			line.mesh = box
			line.material_override = mat_marker
			line.position = Vector3(mid_point.x, 0.05, mid_point.z)
			
			# Orient line towards next waypoint
