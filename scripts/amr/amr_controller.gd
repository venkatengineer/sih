...
func _physics_process(delta: float) -> void:
	...
	# Handle collision blocked state recovery check
	if current_state == RobotState.BLOCKED or current_state == RobotState.WAITING:
		velocity = Vector3.ZERO
		stuck_timer += delta
		if stuck_timer > 2.0:
			stuck_timer = 0.0
	...
	# Physics-aware velocity movement
	velocity = move_dir * move_speed
	velocity.y = 0.0
	
	# Execute Physics Move and Slide
	...
	if get_slide_collision_count() > 0:
		for i in range(get_slide_collision_count()):
			...
			if collider != null:
				# Collided with another AMR or a Static Wall/Rack/Obstacle
				if collider is CharacterBody3D or collider is StaticBody3D:
					var normal = collision.get_normal()
					# If collision normal opposes movement direction, stop and mark BLOCKED
					if move_dir.dot(-normal) > 0.3:
						current_state = RobotState.BLOCKED
						update_status_visuals()
						velocity = Vector3.ZERO
						break
